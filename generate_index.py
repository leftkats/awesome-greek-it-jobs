...
<head>
    ...
    <style>
        ...
        .dark .brutalist-border { border: 1px solid #ffffff; }
        ...
        .dark .dropdown-menu { background: #000000; }
        .show { display: block !important; }
    </style>
</head>
<body class="bg-background-light dark:bg-background-dark text-[#111318] dark:text-white flex flex-col min-h-screen transition-colors duration-200">

<div class="bg-black text-white px-4 py-3 brutalist-border border-x-0 border-t-0 flex items-start gap-3 relative z-[60]">
...
<nav class="sticky top-0 z-50 bg-background-light dark:bg-background-dark brutalist-border border-x-0 border-t-0 px-4 py-4 flex items-center justify-between">
    <div class="flex items-center gap-2">
        <span class="material-symbols-outlined text-xl">database</span>
        <span class="font-bold text-xs mono-text">AGTJ</span>
    </div>
    <div class="flex items-center gap-4">
        <button onclick="document.documentElement.classList.toggle('dark')" class="flex items-center justify-center">
...
<section class="p-4 space-y-4 relative z-40">
    <div class="flex brutalist-border bg-white dark:bg-black overflow-hidden">
        ...
        <input id="searchInput" class="w-full p-3 text-xs mono-text border-none focus:ring-0 bg-transparent" placeholder="Filter by company or sector..." type="text" onkeyup="handleSearch()">
    </div>
...
<main class="flex flex-col flex-grow relative z-10" id="companyList">
    ...
    <div class="company-row flex p-4 brutalist-border border-t-0 border-l-0 border-r-0 bg-white dark:bg-background-dark"
         ...
         data-policy="{{ company.work_policy }}"
         data-sectors="{{ company.sectors|join(' ') }}">
        <div class="flex-1 space-y-2">
            <div class="flex items-center gap-2">
                <h2 class="text-xl font-extrabold uppercase leading-none">{{ company.name }}</h2>
                <span class="text-[10px] brutalist-border px-1 font-bold uppercase {{ get_style(company.work_policy) }}">
                    {{ company.work_policy }}
                </span>
            </div>
            <div class="flex flex-wrap gap-1">
                {% for sector in company.sectors %}
                <span class="text-[10px] mono-text uppercase border border-gray-300 dark:border-gray-700 px-1">{{ sector }}</span>
                {% endfor %}
            </div>
        </div>
        <div class="flex items-center justify-end">
            <a class="brutalist-border p-3 flex items-center justify-center hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors" href="{{ company.careers_url }}" target="_blank">
...
<footer class="p-8 flex flex-col items-center justify-center space-y-8 bg-white dark:bg-background-dark brutalist-border border-l-0 border-r-0 border-b-0">
    <div class="flex gap-2" id="paginationControls">
        <button onclick="changePage(-1)" id="prevBtn" class="brutalist-border px-4 py-2 font-bold mono-text text-xs hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors disabled:opacity-30">Prev</button>
        <div id="pageIndicator" class="brutalist-border px-4 py-2 font-bold mono-text text-xs bg-black text-white dark:bg-white dark:text-black">01</div>
        <button onclick="changePage(1)" id="nextBtn" class="brutalist-border px-4 py-2 font-bold mono-text text-xs hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors disabled:opacity-30">Next</button>
    </div>
    <div class="flex flex-col items-center gap-4">
        <a class="brutalist-border px-6 py-3 font-bold mono-text text-xs flex items-center gap-2 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors" href="https://github.com/leftkats/awesome-greek-tech-jobs">
            <span class="material-symbols-outlined text-sm">terminal</span>
            VIEW ON GITHUB
        </a>
        <div class="text-center">
            <p class="text-[9px] font-bold mono-text opacity-40 uppercase tracking-tighter">Deployed via GitHub Pages</p>
            <p class="text-[9px] font-bold mono-text opacity-40 uppercase tracking-tighter">Maintained by the Community</p>
        </div>
    </div>
</footer>

<div class="fixed bottom-6 right-6 z-[100]">
      <a href="https://github.com/leftkats/awesome-greek-tech-jobs/blob/main/CONTRIBUTING.MD" 
         target="_blank" 
